...
package com.kapouta.katools.imports.texample;

/**
 * Whole package was actually imported from https://github.com/jodem/glText-Android
 * 
 Rendering Text Library in OpenGL 1.1 on Android
 ------------------------------------------------
 This library has been originally released on fractious games web page.

 You CAN use it freely in any commercial, non-commerical,
 open source, or closed source project(s).

 More details about implementation : http://fractiousg.blogspot.fr/2012/04/rendering-text-in-opengl-on-android.html
 * 
 * */
...
public class GLText {
	...
	public boolean load(String file, int size, int padX, int padY) {
		...
		gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MIN_FILTER,
				GL10.GL_NEAREST); // Set Minification Filter
		gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MAG_FILTER,
				GL10.GL_LINEAR); // Set Magnification Filter
		...
		GLUtils.texImage2D(GL10.GL_TEXTURE_2D, 0, bitmap, 0); // Load Bitmap to
																// Texture
	...
	public void draw(String text, float x, float y, float z) {
		...
		for (int i = 0; i < len; i++) { // FOR Each Character in String
			...
			batch.drawSprite(x, y, chrWidth, chrHeight, charRgn[c], z); // Draw
																		// the
																		// Character
	...
	public float drawC(String text, float x, float y, float z) {
		...
		draw(text, x - (len / 2.0f), y - (getCharHeight() / 2.0f), z); // Draw
																		// Text
																		// Centered
	...
	public void drawTexture(int width, int height, float z) {
		...
		batch.drawSprite(textureSize / 2, height - (textureSize / 2),
				textureSize, textureSize, textureRgn, z); // Draw
		batch.endBatch(); // End Batch
	}
